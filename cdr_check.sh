...
CDR_DIRECTORY=""
input=("$@")
command="grep $1 $CDR_DIRECTORY"

for i in ${input[@]}; do
    command="$command | grep \"$i\""
done
...
function Attempt_IP_IP() {
    ...
    local egressTrunk=$(awk -F',' '{print $28}' <<<"$cdr" | awk -F ':' '{print $2}')
    local ingressLocIP=$(awk -F',' '{print $285}' <<<"$cdr")
    local ingressRemIP=$(awk -F',' '{print $286}' <<<"$cdr")
...
function Start_IP_IP() {
    ...
    local egressTrunk=$(awk -F',' '{print $26}' <<<"$cdr" | awk -F':' '{print $2}')
    local IngressTrunk=$(awk -F',' '{print $29}' <<<"$cdr")
    local ingressLocIP=$(awk -F',' '{print $272}' <<<"$cdr")
    local ingressRemIP=$(awk -F',' '{print $273}' <<<"$cdr")
...
function Stop_IP_IP() {
    ...
    local SIPdisconnection=$(awk -F',' '{print $172}' <<<"$cdr")
    ...
    local egressTrunk=$(awk -F',' '{print $31}' <<<"$cdr" | awk -F':' '{print $2}')
    local IngressTrunk=$(awk -F',' '{print $34}' <<<"$cdr")
    local ingressLocIP=$(awk -F',' '{print $295}' <<<"$cdr")
    local ingressRemIP=$(awk -F',' '{print $296}' <<<"$cdr")
...
while IFS= read -r line; do
    record_type=$(awk -F',' '{print $1}' <<<"$line" | awk -F':' '{print $2}')
    echo "------------------------------------------------------------------------------------------------------------------------------"
    case "$record_type" in
    "ATTEMPT")
